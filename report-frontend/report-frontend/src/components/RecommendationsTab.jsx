...
const RecommendationsTab = ({
  ...
  const [isDownloadButtonHovered, setIsDownloadButtonHovered] = useState(false);
  const [isDownloadButtonPressed, setIsDownloadButtonPressed] = useState(false);
  // State for Download DOCX button
  const [isLoadingDocx, setIsLoadingDocx] = useState(false);
  const [isDocxButtonHovered, setIsDocxButtonHovered] = useState(false);
  const [isDocxButtonPressed, setIsDocxButtonPressed] = useState(false);
  ...
  const handleDownload = async () => {
    ...
    formData.append("reasonForReferral", values.reasonForReferral || "");
    formData.append("testsadministered", values.testsadministered);
    formData.append("otherTest", values.otherTest);
    formData.append("readingAge", values.readingAge);
    formData.append("spellingAge", values.spellingAge);
  ...
  const generateSummary = () => {
    const viq = localStorage.getItem("verbalIQ");
    const piq = localStorage.getItem("performanceIQ");
    const tiq = localStorage.getItem("OverallTQ");
    const name = getValues("name") || "The child";
    const gender = getValues("gender") || "";
    const pronouns = getPronouns(gender);
    const vClass = classifyScore(viq);
    const pClass = classifyScore(piq);
    const tClass = classifyScore(tiq);
    let summary = `${name} was administered the Wechsler Intelligence Scale. `;
    if (viq) summary += `${pronouns.he_she.charAt(0).toUpperCase() + pronouns.he_she.slice(1)} obtained a Verbal IQ (VIQ) of ${viq}, which falls in the ${vClass} range. `;
    if (piq) summary += `${pronouns.he_she.charAt(0).toUpperCase() + pronouns.he_she.slice(1)} obtained a Performance IQ (PIQ) of ${piq}, which falls in the ${pClass} range. `;
    if (tiq) summary += `The Total IQ score is ${tiq}, indicating ${tClass} level of intellectual functioning.`;
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      ...
      alert("DOCX download failed. Check console for more info.");
    } finally {
      ...
      <div className="flex flex-wrap justify-between gap-4 mt-6">
        ...
        <div className="flex flex-1 sm:flex-auto justify-end gap-x-4">
          ...
          <button
            ...
            className="px-6 py-3 rounded transition"
            style={{
              backgroundColor: isDocxButtonHovered ? "#4338ca" : "#4f46e5",
              color: "white",
              transform: isDocxButtonPressed ? "scale(0.98)" : "scale(1)",
            }}
            onClick={handleDownloadDocx}
            disabled={isLoadingDocx}
            onMouseEnter={() => setIsDocxButtonHovered(true)}
            onMouseLeave={() => setIsDocxButtonHovered(false)}
            onMouseDown={() => setIsDocxButtonPressed(true)}
            onMouseUp={() => setIsDocxButtonPressed(false)}
          >
            {isLoadingDocx ? (
              <div
                style={{
              ...
              >
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                <span className="ml-2">Generating DOCX...</span>
              </div>
            ) : (
              "Download as DOCX"
            )}

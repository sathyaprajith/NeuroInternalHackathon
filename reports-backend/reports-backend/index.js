...
const buildReplacements = (body) => {
  ...
  return {
    ...
    "«Referred_By»": body.referredBy || "school",
    "«Reason_for_Referral»":
      body.reasonForReferral || "intellectual and educational assessment",
...
app.post("/download-docx", upload.none(), async (req, res) => {
  try {
    const body = req.body;
    const replacements = buildReplacements(body);
    const templatePath = path.join(
      __dirname,
      "template",
      "complete_report.html",
